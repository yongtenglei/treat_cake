...
def test_alex_aviad_same_evaluations_case_slope_graph_one_seg():
    ...
    assert check_if_envy_free(
        4, result, epsilon=to_decimal("1e-5"), preferences=preferences
    ), "Yield none-envy-free allocation"
...
def test_alex_aviad_same_evaluations_case_flat_graph_three_segs():
    ...
    logging.info(f"{result=}")
    assert check_if_envy_free(
        4, result, to_decimal("1e-5"), preferences=preferences
    ), "Yield none-envy-free allocation"
